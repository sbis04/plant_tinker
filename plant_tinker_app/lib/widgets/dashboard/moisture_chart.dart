...
class MoistureChart extends StatelessWidget {
  ...
  MoistureChart({required this.docs});

  final List<Color> gradientColors = [
    Palette.blue_accent,
    Palette.green_accent,
  ];
          ...
          getTitles: (value) {
            ...
            if (value == 0) {
              return formatter.format(DateTime.fromMillisecondsSinceEpoch(
                  (docs[0].data()['timestamp']) * 1000));
            } else if (value == (total - 2)) {
              return formatter.format(DateTime.fromMillisecondsSinceEpoch(
                  (docs[total - 1].data()['timestamp']) * 1000));
            }
          ...
          spots: [
            for (int i = 0; i < docs.length; i++)
              FlSpot(
                i.toDouble(),
                double.parse(
                    (docs[i].data()['moisture'] as double).toStringAsFixed(2)),
              ),
          ],
          isCurved: false,
